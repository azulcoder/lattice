...
//   A. (BIGGEST) The paper's exact property TABLES were not recovered (full text
//      403'd). Every numeric range here is from SECONDARY literature — confirm
//      against the actual paper before publishing any specific number. Module is
//      written about the METHOD; specific numbers are flagged.
//   B. G.S. Bodvarsson deceased 2006 (verified) — bio in accomplishment tense;
//      birth year 1952. Grimur Bjornsson living (career tense).
//   C. Survey's vapour-dominated (dry-steam) coverage — confirm before using as a
//      dry-steam kh/porosity plausibility check for Darajat.
//   D. Cross-refs to acuna + cumming are MISSING in the catalog graph (optional adds).
...
The headline finding is a warning against false precision: geothermal permeability is **fracture-controlled and enormously variable** — producing fractures within even a single field can span roughly two orders of magnitude. You cannot assume a tidy value; you have to measure, and you have to expect a wide spread. For a field like the catalog owner's, the survey is the external yardstick against which the field's measured permeability-thickness and porosity can be placed: is this field permeable by world standards, and is a fitted value plausible? **⟦TODO-Az: the survey's own tabulated ranges were not recoverable here (the paper's full text was paywalled), so every specific number in this module is from secondary literature and must be confirmed against the actual paper — and where Darajat's measured kh/porosity sit relative to the survey is yours to supply. Prep §9-A, §9-C.⟧**`,
...
Temuan headline-nya peringatan lawan false precision: permeability geothermal itu **fracture-controlled dan sangat variabel** — producing fracture bahkan dalam satu field bisa span kira-kira dua orders of magnitude. Kamu gak bisa mengasumsikan angka rapi; kamu harus ukur, dan kamu harus expect spread lebar. Buat field seperti punya owner katalog, survey ini yardstick eksternal yang permeability-thickness dan porosity terukur field bisa ditempatkan terhadapnya: apakah field ini permeabel menurut standar dunia, dan apakah nilai yang di-fit plausibel? **⟦TODO-Az: range tabulasi survey-nya sendiri gak bisa di-recover di sini (full text paper paywalled), jadi tiap angka spesifik di module ini dari literatur sekunder dan harus dikonfirmasi lawan paper sebenarnya — dan di mana kh/porosity terukur Darajat duduk relatif ke survey itu punya kamu buat sediain. Prep §9-A, §9-C.⟧**`
      }
    },
...
so, at equal drawdown and geometry, well productivity is *linear* in $kh$. This is the bridge from the surveyed property to deliverability: the two-orders-of-magnitude spread in $kh$ across producing fractures translates directly into a ~100× spread in well output. **⟦TODO-Az: this radial-Darcy PI is the liquid form; a dry-steam well's inflow is compressible (pressure-squared) and often non-Darcy, so the linear-in-kh scaling is an order-of-magnitude guide, not an exact relation — see the Acuña module. Prep §9-C.⟧**
...
jadi, di drawdown dan geometry yang sama, well productivity itu *linear* di $kh$. Ini jembatan dari surveyed property ke deliverability: spread dua-orders-of-magnitude di $kh$ across producing fracture translate langsung jadi spread ~100× di well output. **⟦TODO-Az: radial-Darcy PI ini bentuk liquid; inflow dry-steam well compressible (pressure-squared) dan sering non-Darcy, jadi scaling linear-di-kh itu panduan order-of-magnitude, bukan relasi eksak — lihat module Acuña. Prep §9-C.⟧**
