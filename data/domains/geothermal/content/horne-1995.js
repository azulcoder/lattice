...
//
// VERIFIED (general, cleared against the literature): the PTA backbone and its
// bibliography (Theis line-source, Horner buildup, van Everdingen–Hurst skin/
// storage, Bourdet derivative), the field-unit slope constants (162.6 semilog,
// 70.6 natural-log Ei form, qBμ/kh), Horne's bio/affiliation and book editions,
// and that the geothermal/dry-steam adaptations hold AS GENERAL METHOD —
// two-phase phase-boundary semilog segments (Grant; Garg), real-gas pseudo-
// pressure for vapour-dominated steam (Mannon–Atkinson), and Acuña dry-steam
// deliverability. Those general claims no longer carry flags.
//
// OPEN TODO-Az ITEMS (remaining — all DARAJAT-SPECIFIC, owner sign-off only):
//   A. HEADLINE finalization: confirm the dry-steam Darajat adaptation checklist
//      (compressible/pseudo-pressure, condensation, Acuña, feedzone/fracture,
//      reinjection thermal effects) is the right emphasis for the field.
//   B. Injection / fall-off testing for Darajat reinjectors — the cold-water
//      temperature correction and how injection testing is framed vs production.
//   C. Silica/calcite scaling as the Darajat-specific driver of rising skin —
//      and the cross-link to the geochemistry track.
//   D. The Darajat workflow chain PTA → conceptual model (Cumming) → numerical
//      model (TOUGH2 / O'Sullivan-Pruess-Lippmann).
...
export const CONTENT = {
  ...
  author: {
    ...
    affiliation: {
      en: 'Thomas Davies Barrow Professor of Earth Sciences, Stanford University; Director of the Stanford Geothermal Program. A leading authority on well-test analysis spanning petroleum and geothermal reservoirs.',
      id: 'Thomas Davies Barrow Professor of Earth Sciences, Stanford University; Direktur Stanford Geothermal Program. Otoritas terkemuka di well-test analysis yang ngerentang petroleum dan reservoir geothermal.'
    },
    tagline: {
      en: 'The standard modern guide to reading a reservoir from its pressure response — perturb the well, record p(t), and invert for permeability, skin, and boundaries. Written by the Director of the Stanford Geothermal Program, so the bridge from petroleum PTA to geothermal is built in.',
      id: 'Panduan modern standar buat membaca reservoir dari pressure response-nya — ganggu sumur, rekam p(t), dan inversi buat permeability, skin, dan boundary. Ditulis Direktur Stanford Geothermal Program, jadi jembatan dari PTA petroleum ke geothermal udah ada.'
    },
    bio: {
      en: `Roland Horne is one of the most influential figures in well-test analysis. His book *Modern Well Test Analysis: A Computer-Aided Approach* (1990; 2nd ed. 1995) helped move the field from hand-drawn semilog straight lines to computer-aided, derivative-based interpretation, and it became a standard reference for a generation of reservoir engineers. As Director of the **Stanford Geothermal Program** he sits squarely at the intersection this module needs: the pressure-transient mathematics is shared between petroleum and geothermal, while the *fluid* — high-temperature water that can flash to steam, in fractured, feedzone-dominated rock — is distinctly geothermal.`,
      id: `Roland Horne salah satu figur paling berpengaruh di well-test analysis. Bukunya *Modern Well Test Analysis: A Computer-Aided Approach* (1990; ed. ke-2 1995) ngebantu mindahin field dari semilog straight line gambar-tangan ke interpretasi computer-aided berbasis-derivative, dan jadi referensi standar buat satu generasi reservoir engineer. Sebagai Direktur **Stanford Geothermal Program** dia duduk persis di persimpangan yang module ini butuh: matematika pressure-transient-nya dibagi antara petroleum dan geothermal, sementara *fluida*-nya — air bersuhu-tinggi yang bisa flash jadi steam, di batuan fractured yang didominasi-feedzone — itu khas geothermal.`
    },
    focus: {
      en: `A pressure-transient well test is the reservoir engineer's stethoscope: change the flow at the well by a known amount (start producing, shut it in, or inject), record how the pressure responds over time, and invert that response for what you cannot see directly — the **permeability-thickness (kh)** of the formation, the **skin** (near-well damage or stimulation), the **reservoir pressure**, and the presence of **boundaries or fractures**. The mathematics is the line-source (Theis) solution, read on a semilog plot (the slope gives kh) and, in modern practice, on the **Bourdet pressure-derivative** log-log plot (whose shape diagnoses the flow regime). In a *geothermal* reservoir the same machinery applies but the fluid breaks the textbook assumptions: high temperature makes viscosity and compressibility move; pressure drawdown can flash water to steam, creating a two-phase near-well zone that shows up as a second straight-line segment on the Horner plot (Grant; Garg); production is often from discrete fractures/feedzones, not homogeneous rock; and in a vapour-dominated *dry-steam* reservoir the flow is compressible (analyzed in a real-gas pseudo-pressure, as for natural gas) and well deliverability follows the Acuña relation rather than a liquid productivity index. This module teaches the standard method together with those general geothermal adaptations.`,
      id: `Pressure-transient well test itu stetoskop-nya reservoir engineer: ubah flow di sumur sebesar jumlah yang diketahui (mulai produksi, shut-in, atau injeksi), rekam gimana pressure respond over time, dan inversi response itu buat apa yang gak bisa kamu liat langsung — **permeability-thickness (kh)** formasi, **skin** (damage atau stimulasi near-well), **reservoir pressure**, dan keberadaan **boundary atau fracture**. Matematikanya solusi line-source (Theis), dibaca di plot semilog (slope-nya ngasih kh) dan, di praktik modern, di plot log-log **Bourdet pressure-derivative** (yang bentuknya ngediagnosis flow regime). Di reservoir *geothermal* mesin yang sama berlaku tapi fluidanya ngelanggar asumsi textbook: suhu tinggi bikin viscosity dan compressibility gerak; pressure drawdown bisa flash air jadi steam, ngebikin two-phase near-well zone yang muncul sebagai segmen straight-line kedua di Horner plot (Grant; Garg); produksi sering dari fracture/feedzone diskrit, bukan batuan homogen; dan di reservoir *dry-steam* vapour-dominated flow-nya compressible (dianalisis di real-gas pseudo-pressure, kayak natural gas) dan deliverability sumur ngikut relasi Acuña bukan liquid productivity index. Module ini ngajarin metode standar bareng adaptasi geothermal umum itu.`
    },
    intellectualLineage: {
      en: `The mathematical root is the **line-source / Theis (1935)** solution for radial flow to a well — itself the groundwater analogue of heat conduction — giving the exponential-integral pressure response and its semilog approximation. **Horner (1951)** added the buildup analysis (the Horner plot and extrapolated p*). **van Everdingen and Hurst (1949)** formalized wellbore storage and skin. The modern leap is the **pressure derivative of Bourdet (1983)**, which turned ambiguous semilog fitting into regime diagnosis on a log-log plot — the core of Horne's "computer-aided" approach. The geothermal branch extends this lineage: two-phase well-test theory (Grant; Garg; Pruess and the TOUGH2 lineage, in this catalog via O'Sullivan-Pruess-Lippmann), fractured/feedzone and fractal reservoir models, and the dry-steam deliverability of Acuña.`,
      id: `Akar matematisnya solusi **line-source / Theis (1935)** buat radial flow ke sumur — yang itu sendiri analog groundwater dari konduksi panas — ngasih pressure response exponential-integral dan aproksimasi semilog-nya. **Horner (1951)** nambah analisis buildup (Horner plot dan p* yang diekstrapolasi). **van Everdingen dan Hurst (1949)** ngeformalin wellbore storage dan skin. Lompatan modern-nya itu **pressure derivative Bourdet (1983)**, yang ngubah fitting semilog yang ambigu jadi diagnosis regime di plot log-log — inti dari pendekatan "computer-aided" Horne. Cabang geothermal-nya nerusin lineage ini: teori well-test two-phase (Grant; Garg; Pruess dan garis keturunan TOUGH2, di katalog ini lewat O'Sullivan-Pruess-Lippmann), model reservoir fractured/feedzone dan fractal, dan deliverability dry-steam Acuña.`
    },
    keyCollaborators: {
      en: `Horne's milieu is the Stanford Geothermal Program and the well-test community. The methodological touchstones invoked here are **Dominique Bourdet** (the pressure derivative), **Charles V. Theis** and **D.R. Horner** (the classical solutions), and **Alain Gringarten** (type curves). On the geothermal side the relevant names are **Sabodh Garg** and **Karsten Pruess** (two-phase well-test and simulation), **Malcolm Grant** (Grant-Bixley, in the catalog), and the Stanford Geothermal Workshop community.`,
      id: `Lingkungan Horne itu Stanford Geothermal Program dan komunitas well-test. Tonggak metodologis yang dipanggil di sini itu **Dominique Bourdet** (pressure derivative), **Charles V. Theis** dan **D.R. Horner** (solusi klasik), dan **Alain Gringarten** (type curve). Di sisi geothermal nama yang relevan itu **Sabodh Garg** dan **Karsten Pruess** (well-test dan simulasi two-phase), **Malcolm Grant** (Grant-Bixley, udah di katalog), dan komunitas Stanford Geothermal Workshop.`
    },
    legacy: {
...
Geothermal departs from the petroleum textbook in four ways, all rooted in the fluid. (1) High temperature: water's viscosity and compressibility change strongly with $T$, so the numeric constants in the equations are not the room-temperature petroleum ones. (2) Flashing: drop the pressure enough and water boils to steam near the well, creating a two-phase zone — on a Horner plot this often appears as TWO straight-line segments, a near-well two-phase mobility and a far-field single-phase one, with the single-phase formation property read from the outer segment (Grant; Garg). (3) Fractures/feedzones: geothermal wells frequently produce from a few discrete feedzones, not a uniform porous blanket, so the homogeneous-radial model is an idealization. (4) Dry steam: a vapour-dominated reservoir is compressible-gas flow (analyzed in a real-gas pseudo-pressure, as for natural gas), the near-well process is condensation rather than flashing, and well output follows the Acuña deliverability relation, not a liquid productivity index. The rest of the module builds on this general physics.`,
...
Geothermal berangkat dari textbook petroleum di empat hal, semuanya berakar di fluidanya. (1) Suhu tinggi: viscosity dan compressibility air berubah kuat sama $T$, jadi konstanta numerik di persamaan bukan yang petroleum suhu-ruang. (2) Flashing: turunin pressure cukup dan air mendidih jadi steam near-well, ngebikin two-phase zone — di Horner plot ini sering muncul sebagai DUA segmen straight-line, mobility two-phase near-well dan single-phase far-field, dengan properti formasi single-phase dibaca dari segmen luar (Grant; Garg). (3) Fracture/feedzone: sumur geothermal sering produksi dari beberapa feedzone diskrit, bukan selimut porous seragam, jadi model homogeneous-radial itu idealisasi. (4) Dry steam: reservoir vapour-dominated itu compressible-gas flow (dianalisis di real-gas pseudo-pressure, kayak natural gas), proses near-well-nya kondensasi bukan flashing, dan output sumur ngikut relasi deliverability Acuña, bukan liquid productivity index. Sisa module ini ngebangun di atas fisika umum ini.`
      }
    },
...
Intuisi geothermal nambah lipatan yang layak penekanannya sendiri: gelombang pressure bisa ngubah FLUIDA pas dia lewat. Near sumur geothermal yang berproduksi pressure bisa jatuh di bawah titik didih, jadi air flash jadi steam dan region near-well jadi two-phase, dengan compressibility dan mobility yang sangat beda. Di plot diagnostik ini kenapa kamu bisa liat segmen near-well (two-phase) yang beda dari far-field (single-phase liquid). Buat field dry-steam gambarnya kebalik lagi: fase mobile-nya steam, dan kondensasi, bukan boiling, proses near-well-nya.`
      }
    },
...
The slope is *the* measurement: a smaller slope ⇒ larger $kh$ ⇒ a more deliverable well. The proportionality becomes an equation once a unit system is fixed. In oilfield units the semilog drawdown slope is $m = 162.6\,q B \mu/(kh)$ (psi per log-cycle), the same constant that appears as the companion natural-log prefactor $70.6\,q B \mu/(kh)$ in the line-source pressure equation; consistent SI / dimensionless forms drop these conversion factors, and a geothermal analysis recasts them for the working fluid.
...
**The geothermal extensions.** The equations above are the fluid-neutral backbone; the geothermal literature adapts them in four ways.
  1. **Two-phase near-well.** When drawdown flashes water to steam, the near-well zone is two-phase. The two-phase diffusivity equation still yields a semilog straight line, but its slope is inversely proportional to the *total kinematic mobility* rather than to $k h/\mu$ alone; with a phase boundary at a radius from the well, the Horner/falloff plot shows two straight-line segments — a near-well two-phase one and a far-field single-phase one — and the single-phase formation property is read from the outer (far-field) segment (Grant; Garg).
  2. **Compressible / dry-steam flow.** In a vapour-dominated dry-steam reservoir the fluid is highly compressible, so analysis uses a real-gas **pseudo-pressure** $m(p)$ (or pressure-squared) rather than pressure directly, exactly as for natural-gas wells; the substitution linearizes the diffusivity equation and the constants change accordingly.
  ...
  4. **Injection / fall-off tests.** Cold-water injection (common in geothermal reinjection) changes near-well viscosity and creates a moving thermal front, so injectivity is not simply the mirror of productivity — a temperature correction is needed.`,
...
$$m \\;\\propto\\; \\frac{q\\,\\mu}{k h}\\quad\\Longrightarrow\\quad kh \\;\\propto\\; \\frac{q\\,\\mu}{m}.$$
Slope-nya *itu* pengukurannya: slope lebih kecil ⇒ $kh$ lebih besar ⇒ sumur lebih deliverable. Proporsionalitas-nya jadi persamaan begitu sistem unit ditetapin. Di oilfield unit slope drawdown semilog itu $m = 162.6\,q B \mu/(kh)$ (psi per log-cycle), konstanta yang sama yang muncul sebagai prefaktor natural-log pendamping $70.6\,q B \mu/(kh)$ di persamaan pressure line-source; bentuk SI / tanpa-dimensi yang konsisten ngebuang faktor konversi ini, dan analisis geothermal nyetelnya ulang buat fluida kerjanya.
...
**Ekstensi geothermal-nya.** Persamaan di atas itu backbone netral-fluida; literatur geothermal ngadaptasinya di empat hal.
  1. **Two-phase near-well.** Pas drawdown flash air jadi steam, zona near-well jadi two-phase. Persamaan diffusivity two-phase tetap ngasih straight line semilog, tapi slope-nya berbanding terbalik sama *total kinematic mobility*, bukan sama $k h/\mu$ doang; dengan phase boundary di suatu radius dari sumur, plot Horner/falloff nunjukin dua segmen straight-line — satu two-phase near-well dan satu single-phase far-field — dan properti formasi single-phase dibaca dari segmen luar (far-field) (Grant; Garg).
  2. **Compressible / dry-steam flow.** Di reservoir dry-steam vapour-dominated fluidanya sangat compressible, jadi analisis pakai **pseudo-pressure** real-gas $m(p)$ (atau pressure-squared) bukan pressure langsung, persis kayak sumur natural-gas; substitusinya nge-linearin persamaan diffusivity dan konstantanya berubah sesuai.
  ...
  4. **Injection / fall-off test.** Injeksi cold-water (umum di reinjeksi geothermal) ngubah viscosity near-well dan ngebikin thermal front yang gerak, jadi injectivity bukan sekadar cermin productivity — koreksi temperatur dibutuhin.`
      }
    },
    ...
    // ─────────────── 4. worked-example ───────────────
    {
      ...
      body: {
        en: `**A Horner buildup, end to end (illustrative numbers).** Every number here is a placeholder chosen to show the *procedure*; they are teaching figures, not data from any particular field.
...
**How the geothermal version diverges.** A geothermal buildup changes the procedure in two general ways:
  • If the buildup shows TWO straight-line segments, the near-well (steeper) segment is the two-phase region and the far one the single-phase formation — interpret the formation $kh$ from the far-field segment.
  • For a dry-steam well, replace the pressure axis with a real-gas pseudo-pressure (or pressure-squared), and pair the $kh$/skin from PTA with an Acuña deliverability (output-vs-WHP) curve for the production picture — they answer different questions (reservoir property vs well output).
A real worked example would substitute field data in consistent units; this one is kept fluid-neutral on purpose.`,
        id: `**Horner buildup, ujung ke ujung (angka ilustratif).** Tiap angka di sini placeholder yang dipilih buat nunjukin *prosedur*-nya; ini angka pengajaran, bukan data dari field tertentu mana pun.
...
**Gimana versi geothermal-nya menyimpang.** Buildup geothermal ngubah prosedurnya di dua hal umum:
  • Kalau buildup-nya nunjukin DUA segmen straight-line, segmen near-well (lebih curam) itu region two-phase dan yang jauh single-phase formation — interpretasi $kh$ formasi dari segmen far-field.
  • Buat sumur dry-steam, ganti sumbu pressure dengan real-gas pseudo-pressure (atau pressure-squared), dan pasangin $kh$/skin dari PTA dengan kurva deliverability Acuña (output-vs-WHP) buat gambaran produksi — mereka ngejawab pertanyaan berbeda (properti reservoir vs output sumur).
Worked example nyata bakal nyubstitusi data field di unit yang konsisten; yang ini sengaja dijaga netral-fluida.`
      }
    },
...
**The geothermal-specific payoff.** The reason PTA matters differently in geothermal is the fluid. (1) Two-phase tests yield both a near-well two-phase mobility and the formation kh — and the location of the flashing front itself is information. (2) In a dry-steam field, PTA in pseudo-pressure characterizes the steam reservoir, while Acuña-style deliverability characterizes the well; an engineer needs both. (3) Fracture/feedzone dominance means a single "kh" is an effective average over a few permeable features, to be read with that caveat.
...
**Batas yang jujur.** Well test ngeliat region beberapa puluh sampai ratusan meter di sekitar sumur over tes berjam-jam sampai berhari; dia snapshot lokal, transient, bukan seluruh reservoir, dan inversinya non-unik (model berbeda bisa fit kurva yang sama — derivative nyempitin tapi gak ngilangin ini). Dia ngelengkapi, bukan ngganti, simulasi numerik skala-field (garis keturunan TOUGH2, O'Sullivan-Pruess-Lippmann) yang ngintegrasi banyak sumur dan conceptual model (Cumming).`
    ...
    // ─────────────── 6. practice ───────────────
    {
      ...
      body: {
        en: `These practice items exercise the verified PTA backbone; the answers stay fluid-neutral and note where the geothermal specifics enter.
...
Early-time data are dominated by wellbore storage (the well bore decompressing, so the sand-face rate ≠ surface rate) and near-well skin, not by the formation's radial flow — fitting them would give the wrong slope and hence the wrong kh. You identify the valid radial portion from the pressure DERIVATIVE: wellbore storage shows as a unit-slope line, and you only trust the semilog/Horner straight line once the derivative has reached its flat plateau (infinite-acting radial flow). In a two-phase geothermal test the radial portion may itself split into two-phase (near-well) and single-phase (far-field) segments, with the formation property read from the far-field segment (Grant; Garg).
</details>
...
<details><summary>Answer</summary>
In infinite-acting radial flow the pressure changes linearly with the logarithm of time, and the line-source solution makes the slope proportional to qμ/(kh). So kh ∝ qμ/(slope): the slope is the inverse signature of how easily the formation transmits fluid. A FLATTER line (smaller slope) means a LARGER kh — the pressure barely moves to sustain the flow, i.e. a highly transmissive, strongly deliverable well. A steep line means tight rock. For dry-steam (compressible) flow the linear-in-log relation holds in a real-gas pseudo-pressure (or pressure-squared), not pressure directly — the same straight-line logic applies once the variable is transformed, with a correspondingly different slope-to-kh constant.
</details>
...
A well test is local and transient: it probes a region of tens-to-hundreds of metres around one well over hours-to-days, and its inversion is non-unique (several models can match the same curve). A reservoir simulation (e.g. the TOUGH2 lineage) integrates the whole field, many wells, the conceptual model, and long-term mass/heat balance. They are complementary: well tests supply LOCAL, well-by-well kh and skin (and pressure) as hard data points that CALIBRATE and constrain the field-scale simulation, while the simulation provides the global picture the local test cannot see. ⟦TODO-Az: confirm how Darajat workflow chains PTA → conceptual model (Cumming) → numerical model (O'Sullivan-Pruess-Lippmann).⟧
        ...
        id: `Item latihan ini ngelatih backbone PTA yang terverifikasi; jawabannya tetap netral-fluida dan nyatet di mana spesifik geothermal masuk.
...
<details><summary>Jawaban</summary>
Di infinite-acting radial flow pressure berubah linear sama logaritma waktu, dan solusi line-source bikin slope-nya proporsional ke qμ/(kh). Jadi kh ∝ qμ/(slope): slope-nya sidik jari kebalikan dari seberapa gampang formasi nransmisi fluida. Garis lebih DATAR (slope lebih kecil) berarti kh lebih BESAR — pressure-nya nyaris gak gerak buat nyokong flow, yaitu sumur yang sangat transmisif, deliverable kuat. Garis curam berarti batuan ketat. Buat flow dry-steam (compressible) relasi linear-in-log-nya berlaku di real-gas pseudo-pressure (atau pressure-squared), bukan pressure langsung — logika straight-line yang sama berlaku begitu variabelnya ditransformasi, dengan konstanta slope-ke-kh yang beda sesuai.
</details>
...
- **Well output / deliverability**: [Acuña 2008](item:acuna-2008) is the dry-steam deliverability (output-vs-pressure) counterpart — PTA gives the reservoir property (kh, skin), Acuña gives the well's production curve; a dry-steam engineer needs both.
...
- **Chemistry of skin**: [Arnórsson 2000](item:arnorsson-2000) / [Henley-Truesdell-Barton 1984](item:henley-truesdell-barton-1984) — silica/calcite scaling is a physical driver of rising skin over a well's life (the geochemistry ↔ production link).`,
...
- **Output sumur / deliverability**: [Acuña 2008](item:acuna-2008) itu counterpart deliverability dry-steam (output-vs-pressure) — PTA ngasih properti reservoir (kh, skin), Acuña ngasih kurva produksi sumur; engineer dry-steam butuh keduanya.
...
- **Kimia skin**: [Arnórsson 2000](item:arnorsson-2000) / [Henley-Truesdell-Barton 1984](item:henley-truesdell-barton-1984) — scaling silica/calcite itu pendorong fisik dari skin yang naik sepanjang umur sumur (link geochemistry ↔ produksi).`
      }
    },
    ...
    // ─────────────── 8. sources ───────────────
    {
      ...
      body: {
        en: `Editions and venues below are verified against the public record; page references for a specific edition are added at finalization.
- **Horne, R. N.** (1995). *Modern Well Test Analysis: A Computer-Aided Approach*, 2nd ed. Petroway Inc. (1st ed. 1990.) **(This item.)** The standard modern PTA workflow (derivative-based diagnosis + model identification).
...
- **Bourdet, D., Whittle, T. M., Douglas, A. A., and Pirard, Y. M.** (1983). "A New Set of Type Curves Simplifies Well Test Analysis." *World Oil.* The pressure derivative — later expanded in **Bourdet, Ayoub, and Pirard** (1989), "Use of Pressure Derivative in Well Test Interpretation," *SPE Formation Evaluation.*
- **Grant, M. A., and Bixley, P. F.** (2011). *Geothermal Reservoir Engineering*, 2nd ed. Elsevier. The geothermal well-test counterpart (in this catalog).
- **Garg, S. K.** (1980). "Pressure Transient Analysis for Two-Phase (Water/Steam) Geothermal Reservoirs." *SPE J.* 20: 206–214 (SPE-7479) — the two-phase diffusivity equation; slope inversely proportional to total kinematic mobility. **Grant, M. A.** (1980), "Pressure Transient Analysis of Geothermal Wells with Phase Boundaries" (SPE-9274), and **Garg & Pruess** (1984, *Water Resources Research*) extend it to phase boundaries and numerical results.`,
        id: `Edisi dan venue di bawah diverifikasi terhadap public record; referensi halaman buat edisi spesifik ditambah saat finalisasi.
- **Horne, R. N.** (1995). *Modern Well Test Analysis: A Computer-Aided Approach*, ed. ke-2. Petroway Inc. (ed. ke-1 1990.) **(Item ini.)** Workflow PTA modern standar (diagnosis berbasis-derivative + identifikasi model).
...
- **Bourdet, D., Whittle, T. M., Douglas, A. A., dan Pirard, Y. M.** (1983). "A New Set of Type Curves Simplifies Well Test Analysis." *World Oil.* Pressure derivative — kemudian diperluas di **Bourdet, Ayoub, dan Pirard** (1989), "Use of Pressure Derivative in Well Test Interpretation," *SPE Formation Evaluation.*
- **Grant, M. A., dan Bixley, P. F.** (2011). *Geothermal Reservoir Engineering*, ed. ke-2. Elsevier. Counterpart well-test geothermal (di katalog ini).
- **Garg, S. K.** (1980). "Pressure Transient Analysis for Two-Phase (Water/Steam) Geothermal Reservoirs." *SPE J.* 20: 206–214 (SPE-7479) — persamaan diffusivity two-phase; slope berbanding terbalik sama total kinematic mobility. **Grant, M. A.** (1980), "Pressure Transient Analysis of Geothermal Wells with Phase Boundaries" (SPE-9274), dan **Garg & Pruess** (1984, *Water Resources Research*) ngeluasinnya ke phase boundary dan hasil numerik.`
      }
    },
  ],
